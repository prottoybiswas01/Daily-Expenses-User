...
import { generateSharedLinkApi, getSharedLinksApi, revokeSharedLinkApi } from '../services/guardianService';
import { X, ShieldCheck, Copy, Check, Trash2 } from 'lucide-react';
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-container" style={{ maxWidth: '640px' }} onClick={(e) => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)' }}>Share a read-only reference link with your parents or guardian</p>
            </div>
          </div>
        ...
        <form onSubmit={handleGenerate} style={{ marginBottom: '1.5rem', background: 'rgba(0,0,0,0.15)', padding: '1rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)' }}>
          <h4 style={{ fontSize: '0.9rem', fontWeight: 700, marginBottom: '0.75rem', color: 'var(--emerald)' }}>+ Generate New Observer Access Link</h4>
          <div className="grid-2" style={{ gap: '0.75rem' }}>
            ...
            <div className="form-group" style={{ marginBottom: 0 }}>
              <input type="email" placeholder="Guardian Email" value={recipientEmail} onChange={(e) => setRecipientEmail(e.target.value)} className="form-input" required />
            </div>
          </div>
          <button type="submit" disabled={submitting} className="btn btn-success" style={{ width: '100%', marginTop: '0.75rem', fontSize: '0.85rem' }}>
            {submitting ? 'Generating...' : 'Generate Guardian Code'}
          </button>
        </form>

        <h4 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '0.75rem' }}>Active & Past Reference Links</h4>
        <div style={{ maxHeight: '220px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {links.length === 0 ? (
            ...
            links.map((link) => (
              <div key={link._id} style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border-color)', padding: '0.75rem', borderRadius: 'var(--radius-md)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  <div style={{ fontSize: '0.85rem', fontWeight: 700 }}>{link.recipientName} ({link.recipientEmail})</div>
                  <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                    Code: <strong style={{ color: 'var(--primary)' }}>{link.accessCode}</strong> • {link.status}
                  </div>
                </div>
                <div style={{ display: 'flex', gap: '0.35rem' }}>
                  {link.status === 'Active' && (
                    <button onClick={() => copyToClipboard(link.accessCode)} className="btn btn-secondary" style={{ padding: '0.35rem 0.6rem', fontSize: '0.75rem' }}>
                      {copiedCode === link.accessCode ? <Check size={14} color="var(--emerald)" /> : <Copy size={14} />} Link
                    </button>
                  )}
                  {link.status === 'Active' && (
